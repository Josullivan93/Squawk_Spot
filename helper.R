...
calc_features <- function(wav_input, window_ms = 30, overlap = 0.5) {
  ...
  # Feature Caclulation
  # Pre-calculate MFCCs
  all_mels <- tuneR::melfcc(wav_input,
    numcep = 13, wintime = window_ms / 1000,
  ...
  res <- seewave::spectro(wav_input@left,
    f = sr, wl = window_samps, ovlp = overlap * 100, zp = zp_req, wn = "hanning",
  ...
  colnames(mfcc_df) <- paste0("mfcc", seq_len(ncol(mfcc_df)))
  colnames(mfcc_d1_df) <- paste0("d1_mfcc", seq_len(ncol(mfcc_d1_df)))
  colnames(mfcc_d2_df) <- paste0("d2_mfcc", seq_len(ncol(mfcc_d2_df)))
...
extract_features <- function(original_path, audio_path, p_cfg = list()) {
  ...
  features_df <- calc_features(
    wav_input = cleaned_wave,
...
group_and_slice_chunks <- function(features_df, full_wave, positive_class,
  ...
  if (sum(is_pos) == 0) {
    ...
    empty_runs <- data.table(
      run_id = integer(0), start_time = numeric(0),
  ...
  for (i in seq_len(nrow(runs_dt))) {
    ...
    out_name <- paste0(
      tools::file_path_sans_ext(basename(original_path)),
